...
@section('content')
    <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        <form method="POST" action="{{ route('super-admin.marketplaces.update', $marketplace) }}" class="space-y-4" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-medium text-slate-700">Ad</label>
                <input type="text" name="name" value="{{ old('name', $marketplace->name) }}" class="mt-1 w-full border-slate-300 rounded-md" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700">Kod</label>
                <input type="text" name="code" value="{{ old('code', $marketplace->code) }}" class="mt-1 w-full border-slate-300 rounded-md" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700">API URL</label>
                <input type="url" name="api_url" value="{{ old('api_url', $marketplace->api_url) }}" class="mt-1 w-full border-slate-300 rounded-md">
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700">Logo URL</label>
                <input type="text" name="logo_url" value="{{ old('logo_url', $marketplace->settings['logo_url'] ?? '') }}"
            ...
            <label class="inline-flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1" class="rounded" @checked(old('is_active', $marketplace->is_active))>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('super-admin.marketplaces.index') }}" class="text-slate-500 hover:text-slate-700">
                    Vazgeç
                </a>
            </div>
        </form>

...
@section('content')
    <div class="bg-white rounded-lg shadow p-6 max-w-4xl">
        <form method="POST" action="{{ route('super-admin.customers.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select id="client-select" name="user_id" class="mt-1 w-full">
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" data-name="{{ $client->name }}" data-email="{{ $client->email }}" @selected(old('user_id') == $client->id)>
                            {{ $client->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('super-admin.customers.index') }}" class="text-slate-500 hover:text-slate-700">Vazgeç</a>
            </div>
        </form>
    </div>
@endsection

    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div class="md:col-span-2">
                ...
                <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" class="w-full border-gray-300 rounded-md" placeholder="İsim veya e-posta">
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1">E-posta</label>
                <input type="text" name="email" value="{{ $filters['email'] ?? '' }}" class="w-full border-gray-300 rounded-md" placeholder="mail@domain.com">
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1">Telefon</label>
                <input type="text" name="phone" value="{{ $filters['phone'] ?? '' }}" class="w-full border-gray-300 rounded-md" placeholder="05xx">
            </div>
            <div>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($customers as $customer)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $customer->user?->name ?? '-' }}</td>

    ...
    <div class="panel-card p-6 max-w-3xl">
        ...
        <form method="POST" action="{{ route('super-admin.plans.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2 border-t border-slate-200 pt-4">
                ...
                <p class="text-sm font-semibold text-slate-800 mb-3">Modüller (Whitelist)</p>

                <p class="text-xs text-slate-500 mb-3">

                    Bu liste, `modules` tablosundan (is_active=true) gelir ve plan.features.modules alanına kaydedilir.
                ...
                <div class="space-y-5">
                    ...
                    @foreach($entitlementModuleGroups as $group)
                        ...
                        <div class="rounded-md border border-slate-200 bg-slate-50/50 p-4">
                            ...
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button" class="btn btn-outline text-xs" data-entitlement-modules-select-all="{{ $group['key'] }}">Hepsini seç</button>

                                    <button type="button" class="btn btn-outline text-xs" data-entitlement-modules-clear-all="{{ $group['key'] }}">Kaldır</button>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3" data-entitlement-modules-group="{{ $group['key'] }}">
                                ...
                                @forelse(($group['items'] ?? []) as $moduleKey => $moduleLabel)
                                    ...
                                    <label class="inline-flex items-center gap-2">
                                        ...
                                        <span class="text-xs text-slate-400">{{ $moduleKey }}</span>
            ...
            <div class="md:col-span-2 flex items-center gap-3 mt-2">
                ...
                <a href="{{ route('super-admin.plans.index') }}" class="btn btn-outline-accent">
...
@push('scripts')
    ...
    <script>
        ...
        function setEntitlementGroupChecked(groupKey, checked) {

            const group = document.querySelector(`[data-entitlement-modules-group="${groupKey}"]`);
        ...
        document.querySelectorAll('[data-entitlement-modules-select-all]').forEach((btn) => {

            btn.addEventListener('click', () => setEntitlementGroupChecked(btn.dataset.entitlementModulesSelectAll, true));

        });

        document.querySelectorAll('[data-entitlement-modules-clear-all]').forEach((btn) => {

            btn.addEventListener('click', () => setEntitlementGroupChecked(btn.dataset.entitlementModulesClearAll, false));

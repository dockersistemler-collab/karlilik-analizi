...
@section('content')
    <style>
        ...
        .nk-profit-table thead th:nth-child(6),
        .nk-profit-table thead th:nth-child(7),
        .nk-profit-table thead th:nth-child(8),
        .nk-profit-table thead th:nth-child(9),
        .nk-profit-table tbody td:nth-child(4),
        .nk-profit-table tbody td:nth-child(5),
        .nk-profit-table tbody td:nth-child(6),
        .nk-profit-table tbody td:nth-child(7),
        .nk-profit-table tbody td:nth-child(8),
        .nk-profit-table tbody td:nth-child(9) {
            white-space: nowrap;
        }
        ...
        .nk-profit-table .nk-order-col {
            min-width: 0;
        }
        .nk-profit-table .nk-date-col {
            min-width: 0;
        }
        .nk-profit-table .nk-money-col {
            min-width: 0;
        }
        .nk-profit-table .nk-rate-col {
            min-width: 0;
        }
        ...
        .nk-stock-table .nk-sku-col {
            min-width: 124px;
        }
        .nk-stock-table .nk-product-col {
            min-width: 136px;
        }
        .nk-stock-table .nk-brand-col {
            min-width: 104px;
        }
                            ...
                            <table class="nk-profit-table">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="nk-market-col">Pazaryeri</th>
                                        <th class="text-left nk-order-col">Siparis Numarasi</th>
                                        <th class="text-left nk-date-col">TAR&#304;H</th>
                                        <th class="nk-num-col nk-money-col">Siparis Tutari</th>
                                        <th class="nk-num-col nk-money-col">Kar Tutari</th>
                                        <th class="nk-num-col nk-rate-col">Kar Orani</th>
                                        <th class="nk-num-col nk-rate-col">Kar Marji</th>
                                        <th class="nk-detail-col">Detay</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($orderRows as $index => $row)
                                        @php
                                            ...
                                            $marketplaceKey = \Illuminate\Support\Str::of(trim((string) $marketplaceName))->lower()->ascii()->value();
                                            $marketplaceLogoPath = $marketplaceLogoMap[$marketplaceKey] ?? null;
                                        ...
                                        <tr>
                                                ...
                                                @if($orderImageUrl)
                                                    <span class="nk-stock-thumb-wrap" tabindex="0" role="button" data-nk-preview-src="{{ $orderImageUrl }}" data-nk-preview-alt="{{ $orderNo }}">
                                                        <img
                                                            ...
                                                            data-fallback-src="{{ $orderImageFallbackUrl }}"
                                            ...
                                            <td class="text-center">
                                                @if($marketplaceLogoUrl)
                                                    <span class="nk-market-cell">
                                                        <img src="{{ $marketplaceLogoUrl }}" alt="{{ $marketplaceName }}" class="nk-market-logo">
                                                        <span class="nk-market-name">{{ $marketplaceName }}</span>
                                                    </span>
                                                @else
                                                    <span class="nk-market-name">{{ $marketplaceName }}</span>
                                                @endif
                                            ...
                                            <td>{{ $orderNo }}</td>
                                            <td>{{ $orderDate }}</td>
                                            <td class="nk-num-col">{{ $salePrice }}</td>
                                            <td class="nk-num-col {{ $negative ? 'nk-negative' : 'nk-positive' }}">{{ $profitAmount }}</td>
                                            <td class="nk-num-col {{ $negative ? 'nk-negative' : 'nk-positive' }}">{{ $profitRate }}</td>
                                            <td class="nk-num-col {{ $negative ? 'nk-negative' : 'nk-positive' }}">{{ $profitMargin }}</td>
                                            <td class="nk-detail-col">
                                                <button
                                                    ...
                                                    data-order-number="{{ $orderNo }}"
                                                    data-sale-price="{{ $salePrice }}"
                                                    data-profit-amount="{{ $profitAmount }}"
                                                    data-product-cost="{{ number_format((float) ($breakdown['product_cost'] ?? 0), 2, ',', '.') }} ₺"
                                                    data-commission="{{ number_format((float) ($breakdown['commission_amount'] ?? 0), 2, ',', '.') }} ₺"
                                                    data-shipping-fee="{{ number_format((float) ($breakdown['shipping_fee'] ?? 0), 2, ',', '.') }} ₺"
                                                    data-platform-fee="{{ number_format($platformFeeAmount, 2, ',', '.') }} ₺"
                                                    data-refund-adjustment="{{ number_format((float) ($breakdown['refunds_shipping_adjustment'] ?? 0), 2, ',', '.') }} ₺"
                                                    data-withholding-tax="{{ number_format($withholdingTaxAmount, 2, ',', '.') }} ₺"
                                                    data-sales-vat="{{ number_format((float) ($breakdown['sales_vat_amount'] ?? 0), 2, ',', '.') }} ₺"
                                                    data-vat-rate="{{ number_format((float) ($breakdown['vat_rate_percent'] ?? 0), 2, ',', '.') }} %"
                                                >Detayı Gör</button>
                                            </td>
                                        </tr>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="10" class="text-center text-muted py-4">Siparis bulunamadi.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                            ...
                            <table class="nk-stock-table">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-left">Gorsel</th>
                                        <th class="text-left nk-sku-col">SKU</th>
                                        <th class="text-left nk-product-col">Urun</th>
                                        <th class="text-left nk-brand-col">Marka</th>
                                ...
                                <tbody>
                                    @forelse($stockProducts as $stockProduct)
                                        ...
                                        <tr>
                                            <td class="nk-check-col">
                                                <input type="checkbox" class="nk-stock-row-check" name="selected_stock_rows[]" value="{{ $stockProduct->id }}">
                                            </td>
                                            <td>
                                                @if($stockImageUrl)
                                            ...
                                            <td>{{ $stockProduct->sku ?: '-' }}</td>
                                            <td class="fw-semibold">{{ $stockProduct->name }}</td>
                                            <td>{{ $stockProduct->brand ?: '-' }}</td>
                                            <td>{{ number_format((float) ($stockProduct->cost_price ?? 0), 2, ',', '.') }} {{ $stockProduct->currency ?: 'TRY' }}</td>
                                            ...
                                            <td class="nk-detail-col">
                                                <button
                                                    ...
                                                    data-product-name="{{ $stockProduct->name }}"
                                                    data-stock-quantity="{{ $stockProduct->stock_quantity }}"
                                                    data-brand="{{ $stockProduct->brand ?: '-' }}"
                                                    data-sku="{{ $stockProduct->sku ?: '-' }}"
                                                    data-price="{{ number_format((float) $stockProduct->price, 2, ',', '.') }} {{ $stockProduct->currency ?: 'TRY' }}"
                                                    data-marketplaces="{{ $marketplaceText }}"
                                                    data-critical-level="{{ $stockProduct->critical_stock_level }}"
...
@push('scripts')
    ...
    <script>
        (function () {
            const neKazanirimSettings = @json($neKazanirimSettings ?? []);
            ...
            const categoryCatalog = @json($categoryOptions);
            const sortedCategoryCatalog = [...categoryCatalog].sort((a, b) => {
                const aText = `${a?.path || ''} ${a?.name || ''}`;
                const bText = `${b?.path || ''} ${b?.name || ''}`;
            ...
            const clearCategoryInvalid = () => {
                ...
                if (!String(categoryNameInput?.value ?? '').trim()) {
            ...
            const resolveServiceFee = (grossAmount) => {
                ...
                const found = serviceFeeBrackets.find((row) => {
                    const min = Math.max(0, parseDecimal(row?.min));
            ...
            const calculateSalePrice = () => {
                ...
                const hasManualCommissionRate = String(commissionRateInput?.value ?? '').trim().length > 0;
                if (!hasManualCommissionRate && !String(categoryNameInput?.value ?? '').trim()) {
                    markCategoryInvalid();
                    hasMissingField = true;
                }
                ...
                const productCost = Math.max(0, parseDecimal(productCostInput?.value ?? 0));
                const shippingFee = Math.max(0, parseDecimal(shippingFeeInput?.value ?? 0));
                const targetValue = Math.max(0, parseDecimal(profitTargetValueInput?.value ?? 0));
                const vatRatePercent = Math.max(0, parseDecimal(vatRateCustomInput?.value ?? 0));
                const commissionRatePercent = Math.min(99.99, Math.max(0, parseDecimal(commissionRateInput?.value ?? 0)));

                const desiredProfit = getSelectedProfitType() === 'rate'
                    ? (productCost * targetValue) / 100
                    : targetValue;

                const baseAmount = productCost + shippingFee + desiredProfit;
                const commissionFactor = 1 - (commissionRatePercent / 100);
                const netRequired = commissionFactor > 0 ? (baseAmount / commissionFactor) : baseAmount;
                ...
                const selectedMarketplaceKey = String(marketplacePlatformSelect?.value || 'trendyol').toLowerCase();
                const selectedMarketplaceLabel = marketplacePlatformSelect?.selectedOptions?.[0]?.textContent?.trim() || 'Trendyol';
                const platformServiceAmount = Number(platformServiceAmounts[selectedMarketplaceKey] ?? 0);
            ...
            calculateButton?.addEventListener('click', calculateSalePrice);

            renderCategoryOptions('');
            categorySelectTrigger?.addEventListener('click', () => {
            ...
            const syncTableView = () => {
                const showStockTable = Boolean(stockViewToggle?.checked);
                profitTablePanel?.classList.toggle('is-hidden', showStockTable);
                stockTablePanel?.classList.toggle('is-hidden', !showStockTable);
            ...
            const syncLeftTableHeight = () => {
                ...
                const headerHeight = leftHeader ? leftHeader.getBoundingClientRect().height : 0;
            ...
            stockCalcModal?.querySelectorAll('[data-nk-modal-close]').forEach((btn) => {
            ...
            const placePopover = (event) => {
                if (!imagePopover || !imagePopoverImg) return;
                const offset = 16;
                const width = 150;
                const height = 150;
            ...
            const closeInlineRows = (scope) => {
                ...
                root.querySelectorAll('[data-nk-inline-row]').forEach((detailRow) => {
                    ...
                    detailRow.querySelector('[data-nk-inline-panel]')?.classList.remove('is-open');

                    const btn = detailRow.previousElementSibling?.querySelector('[data-nk-detail]');
                    if (btn) {
            ...
            const captureOpenProfitDetailState = () => {
                ...
                const sourceButton = openDetailRow.previousElementSibling?.querySelector('[data-nk-detail]');
                if (!sourceButton) {
                    return null;
                }
            ...
            const refreshOrdersTable = async () => {
                if (isRefreshingOrders || stockViewToggle?.checked) {
                    return;
                }

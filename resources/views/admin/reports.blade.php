...
@section('content')
    @php
        ...
        $canExport = !$activePlan || $activePlan->hasModule('exports.reports.orders');
    @endphp
    <div class="panel-card p-6 mb-6">
        <form method="GET" class="flex flex-wrap lg:flex-nowrap items-end gap-3">
            ...
            <div class="flex items-center gap-2 lg:ml-auto">
                ...
                <a href="{{ route('admin.reports.index') }}" class="btn btn-outline">Temizle</a>
            </div>
            @if($reportExportsEnabled && $canExport)
                <details class="relative">
                    ...
                    <div class="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-lg shadow-lg p-2 z-10">
                        <a href="{{ route('admin.reports.orders-revenue.export', request()->query()) }}" class="block px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 rounded-md">Siparişleri Excel'e Aktar</a>
                        <a href="{{ route('admin.reports.orders-revenue.invoiced-export', request()->query()) }}" class="block px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 rounded-md">Faturalandırılmış Siparişleri Excel'e Aktar</a>
                    </div>
                </details>
            @endif
        </form>

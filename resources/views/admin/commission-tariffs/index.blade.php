        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm commission-table">
                ...
                <tbody id="commission-table-body">
                    <tr>
                        <td colspan="9" class="text-center text-slate-500 py-6">Yükleniyor...</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<style>
    ...
    .commission-table th:nth-child(9),
    .commission-table td:nth-child(9) {
        width: 170px;
    }
    ...
    .commission-table thead tr th:nth-child(8) {
        text-align: center;
    }
...
<script>
    ...
    const systemFields = [
        ...
        { key: 'range4Min', label: '4. Aralık Min' },
        { key: 'range4Max', label: '4. Aralık Max' },
    ...
    document.getElementById('commission-upload-form')?.addEventListener('submit', async (event) => {
        ...
        const response = await fetch('{{ route('portal.commission-tariffs.api.upload') }}', {
            method: 'POST',
            headers: {
    ...
    mapSave?.addEventListener('click', async () => {
        ...
        await fetch('{{ route('portal.commission-tariffs.api.column-map') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'X-Requested-With': 'XMLHttpRequest',
    ...
    errorsBtn?.addEventListener('click', async () => {
        ...
        const response = await fetch(`{{ url('/api/commission-tariffs/errors') }}/${latestUploadId}`);
        const data = await response.json();
    ...
    async function loadTable(page = 1) {
        ...
        const response = await fetch(`{{ route('portal.commission-tariffs.api.list') }}?${params.toString()}`);
        const data = await response.json();
    ...
    function renderTable(rows) {
        if (!rows.length) {
            tableBody.innerHTML = '<tr><td colspan="9" class="text-center text-slate-500 py-6">Kayıt bulunamadı.</td></tr>';
            return;
        }
    ...
    function bindCalcButtons() {
        document.querySelectorAll('.commission-calc-btn').forEach(btn => {
            btn.addEventListener('click', async (event) => {
                ...
                const response = await fetch('{{ route('portal.commission-tariffs.api.recalc') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'X-Requested-With': 'XMLHttpRequest',
    ...
    document.getElementById('commission-bulk-download')?.addEventListener('click', async () => {
        ...
        const response = await fetch('{{ route('portal.commission-tariffs.api.export') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'X-Requested-With': 'XMLHttpRequest',

...
@section('content')
    ...
    <div class="panel-card p-6 mb-6 report-filter-panel">
        <form id="brand-sales-filters" method="GET" action="{{ route('portal.reports.brand-sales') }}" class="flex flex-wrap lg:flex-nowrap items-end gap-3 report-filter-form">
            ...
            <div class="min-w-[220px] report-filter-field">
                ...
                <input type="hidden" name="quick_range" id="brand-quick-range-input" value="{{ $filters['quick_range'] ?? '' }}">
            </div>

            <div class="report-filter-actions">
                <button type="submit" class="report-filter-btn report-filter-btn-primary">Filtrele</button>
                <a href="{{ route('portal.reports.brand-sales') }}" class="report-filter-btn report-filter-btn-secondary">Temizle</a>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-[12px] table-fixed">
                <thead class="text-xs uppercase text-slate-400">
                    <tr>
                        <th class="text-left py-2 pr-4 w-[30%]">Marka</th>
                        <th class="text-center py-2 pr-4 w-[16%]">Satis Adedi</th>
                        <th class="text-center py-2 pr-4 w-[16%]">Ciro (TL)</th>
                        <th class="text-center py-2 pr-4 w-[18%]">Ort. Sepet (TL)</th>
                        <th class="text-right py-2 w-[20%]">Detay</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100" id="brand-sales-table-body">
                    @forelse(($report['table'] ?? []) as $row)
                        @php
                            ...
                            $marketplaceLines = [];
                            foreach ($marketplaces as $marketplace) {
                                $marketplaceLines[] = $marketplace->name . ': ' . number_format((float) ($row['mp_' . $marketplace->id] ?? 0), 2, ',', '.') . ' TL';
                            }
                        @endphp
                        <tr>
                            <td class="py-2 pr-2.5 text-slate-700 font-semibold">{{ $row['brand'] ?? '-' }}</td>
                            ...
                            <td class="py-3 text-right">
                                <button type="button" class="btn btn-solid-accent px-2.5 py-1 text-[10px]" data-brand-detail-toggle data-brand-name="{{ $row['brand'] ?? '-' }}" data-orders="{{ number_format($orders, 0, ',', '.') }}" data-revenue="{{ number_format($revenue, 2, ',', '.') }} TL" data-average="{{ number_format($avg, 2, ',', '.') }} TL" data-marketplace-lines="{{ e(implode(' | ', $marketplaceLines)) }}">Detayi Gor</button>
                            </td>
                        </tr>
                        <tr class="rp-detail-row is-hidden" data-brand-detail-row>
                            <td colspan="5">
                                <div class="rp-detail-panel">
                                    <div class="rp-detail-title">Marka Satis Detayi</div>
                                    <div class="rp-detail-subtitle" data-detail-field="brand-name">-</div>
                                    ...
                                    <div class="rp-detail-list">
                                        <div class="rp-detail-line"><span>Marka</span><strong class="is-neutral" data-detail-field="brand-name-line">-</strong></div>
...
@push('scripts')
    <script>
        (function () {
            const form = document.getElementById('brand-sales-filters');
            const quickInput = document.getElementById('brand-quick-range-input');
            ...
            const closeAll = (tbody) => {
                tbody.querySelectorAll('[data-brand-detail-row]').forEach((row) => row.classList.add('is-hidden'));
                tbody.querySelectorAll('[data-brand-detail-toggle]').forEach((btn) => btn.textContent = 'Detayi Gor');
            };

            document.querySelectorAll('[data-brand-detail-toggle]').forEach((btn) => {
                btn.addEventListener('click', () => {
                    const row = btn.closest('tr');
                    const detailRow = row?.nextElementSibling;
                    const tbody = row?.parentElement;
                    if (!detailRow || !detailRow.matches('[data-brand-detail-row]') || !tbody) return;
                    ...
                    setText('brand-name', btn.getAttribute('data-brand-name'));
                    setText('brand-name-line', btn.getAttribute('data-brand-name'));
                    ...
                    setText('marketplace-lines', btn.getAttribute('data-marketplace-lines') || '-');

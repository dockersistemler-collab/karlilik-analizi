...
@section('content')
    <div class="panel-card p-6 max-w-3xl">
        <form method="POST" action="{{ route('admin.invoices.store') }}" class="space-y-5">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700">Müşteri Ara</label>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.invoices.index') }}" class="btn btn-outline-accent">Vazgeç</a>
            </div>
        </form>
    </div>
...
<script>
    ...
    searchInput?.addEventListener('input', () => {
        ...
        timer = setTimeout(async () => {
            const response = await fetch(`{{ route('admin.invoices.customers') }}?q=${encodeURIComponent(query)}`);
    ...
    modalForm?.addEventListener('submit', async (event) => {
        ...
        let contentType = '';
        try {
            response = await fetch('{{ route('admin.customers.store') }}', {
                method: 'POST',
                headers: {

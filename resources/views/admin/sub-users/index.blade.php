                    ...
                    @forelse($subUsers as $subUser)
                        <tr class="border-t border-slate-100">
                            ...
                            <td class="text-right whitespace-nowrap">
                                <a href="{{ route('admin.sub-users.edit', $subUser) }}" class="text-slate-600 hover:text-slate-900 mr-3">Düzenle</a>
                                <form method="POST" action="{{ route('admin.sub-users.destroy', $subUser) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
...
<script>
    ...
    subuserForm?.addEventListener('submit', async (event) => {
        ...
        const response = await fetch('{{ route('admin.sub-users.store') }}', {
            method: 'POST',
            headers: {
        ...
        if (tableBody && payload) {
            ...
            row.innerHTML = `
                ...
                <td class="text-right whitespace-nowrap">
                    <a href="{{ url('/admin/sub-users') }}/${payload.id}/edit" class="text-slate-600 hover:text-slate-900 mr-3">Düzenle</a>
                    <form method="POST" action="{{ url('/admin/sub-users') }}/${payload.id}" class="inline">

...
@section('content')
    <form method="POST" action="{{ route('portal.sub-users.store') }}" class="panel-card p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="email" name="email" class="mt-1 w-full" value="{{ old('email') }}" required>
                @error('email')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2 flex items-center gap-2">
                <input type="checkbox" id="is_active" name="is_active" value="1" class="rounded" {{ old('is_active', true) ? 'checked' : '' }}>
                ...
                @foreach($permissions as $key => $label)
                    <label class="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2">
                        <input type="checkbox" name="permissions[]" value="{{ $key }}" class="rounded"
                            {{ in_array($key, old('permissions', ['dashboard'])) ? 'checked' : '' }}>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('portal.sub-users.index') }}" class="text-slate-500 hover:text-slate-700">Vazgeç</a>
        </div>
    </form>
@endsection

    ...
    <div class="panel-card p-6 space-y-4">
        ...
        @if($marketplaces->isEmpty())
            <div class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                ...
                <a href="{{ route('admin.integrations.index') }}" class="font-semibold underline">Mağaza Bağla</a>
            </div>
        @endif

        <form method="GET" action="{{ route('admin.categories.index') }}" class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
...
<script>
    ...
    categoryForm?.addEventListener('submit', async (e) => {
        ...
        const actionUrl = isEdit ? `{{ url('/admin/categories') }}/${categoryId}` : '{{ route('admin.categories.store') }}';
    ...
    document.querySelectorAll('.mp-search').forEach((input) => {
        ...
        input.addEventListener('input', () => {
            ...
            debounce(`${categoryId}:${marketplaceId}`, async () => {
                const response = await fetch(`{{ url('/admin/marketplace-categories') }}/${marketplaceId}/search?q=${encodeURIComponent(q)}`, { headers: { 'Accept': 'application/json' } });
                const payload = await response.json().catch(() => null);

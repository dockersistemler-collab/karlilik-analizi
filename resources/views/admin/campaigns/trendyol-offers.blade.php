...
<script>
    ...
    document.getElementById('commission-upload-form')?.addEventListener('submit', async (event) => {
        ...
        const response = await fetch('{{ route('portal.campaigns.trendyol-offers.api.upload') }}', {
            method: 'POST',
            headers: {
    ...
    mapSave?.addEventListener('click', async () => {
        ...
        await fetch('{{ route('portal.campaigns.trendyol-offers.api.column-map') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'X-Requested-With': 'XMLHttpRequest',
    ...
    errorsBtn?.addEventListener('click', async () => {
        ...
        const response = await fetch(`{{ url('/api/campaigns/trendyol-offers/errors') }}/${latestUploadId}`);
        const data = await response.json();
    ...
    function bindCalcButtons() {
        document.querySelectorAll('.commission-calc-btn').forEach(btn => {
            btn.addEventListener('click', async (event) => {
                ...
                const response = await fetch('{{ route('portal.campaigns.trendyol-offers.api.recalc') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'X-Requested-With': 'XMLHttpRequest',

...
class HepsiburadaAdapter extends BaseMarketplaceAdapter
{
    ...
    public function fetchOrders(MarketplaceAccount $account, DateRange $range): iterable
    {
        ...
        if ($baseUrl === '' || !$this->hasAuth($account)) {
            Log::warning('Hepsiburada orders fetch skipped: missing credentials/base_url', [
        ...
        $query = [
            // TODO: Hepsiburada order API tarih filtreleri: dokümana göre startDate/endDate formatı doğrulanacak.
        ...
        if (!$response->successful()) {
            Log::warning('Hepsiburada orders fetch failed', [
                'tenant_id' => $account->tenant_id,
                'account_id' => $account->id,
    ...
    public function fetchReturns(MarketplaceAccount $account, DateRange $range): iterable
    {
        ...
        $query = [
            // TODO: Hepsiburada returns API tarih filtreleri: dokümana göre format doğrulanacak.
            'startDate' => $range->from->toDateTimeString(),
            'endDate' => $range->to->toDateTimeString(),
        ];
        ...
        $response = $this->authRequest($account)->get($url, $query);
        if (!$response->successful()) {
            Log::warning('Hepsiburada returns fetch failed', [
                'tenant_id' => $account->tenant_id,
                'account_id' => $account->id,
                'status' => $response->status(),
            ]);
    ...
    public function fetchFees(MarketplaceAccount $account, DateRange $range): iterable
    {
        ...
        $query = [
            // TODO: Hepsiburada fees API tarih filtreleri: dokümana göre format doğrulanacak.
            'startDate' => $range->from->toDateTimeString(),
            'endDate' => $range->to->toDateTimeString(),
        ];
        ...
        $response = $this->authRequest($account)->get($url, $query);
        if (!$response->successful()) {
            Log::warning('Hepsiburada fees fetch failed', [
                'tenant_id' => $account->tenant_id,
                'account_id' => $account->id,
                'status' => $response->status(),
            ]);

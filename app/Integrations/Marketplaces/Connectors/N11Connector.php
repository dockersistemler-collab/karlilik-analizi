...
class N11Connector implements MarketplaceConnectorInterface
{
    ...
    public function updateStock(MarketplaceListing $listing, int $quantity): array
    {
        ...
        $creds = $this->credentials($account);
        $client = $this->client($creds);
        $url = $this->resolveEndpoint($creds, 'stock_update_url', '/products/stock');
        ...
        if (!$response->successful()) {
            Log::warning('inventory.connector.n11.update_stock.failed', [
        ...
        return ['ok' => $response->successful(), 'status' => $response->status(), 'provider' => 'n11'];
    }

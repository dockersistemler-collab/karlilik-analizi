...
class HepsiburadaConnector implements MarketplaceConnectorInterface
{
    ...
    public function updateStock(MarketplaceListing $listing, int $quantity): array
    {
        ...
        if (!$response->successful()) {
            Log::warning('inventory.connector.hepsiburada.update_stock.failed', [
        ...
        return ['ok' => $response->successful(), 'status' => $response->status(), 'provider' => 'hepsiburada'];
    }
